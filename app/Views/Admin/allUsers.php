    ...
    <div class="row">
      <div class="col-md-12 col-lg-8">
        <div class="title-single-box">
          ...
          <span class="color-text-a">List of all users</span>
        </div>
      </div>
    </div>
  </div>
  ...
  <table class="table table-hover" id="myTable">
    ...
    <tbody>
      <?php foreach ($users as $user) : ?>
        ...
        <td><?= $user->nomClient . " " . $user->prenomClient ;?></td>
        <td><?= $user->emailUser ;?></td>
        <td><?= $sexe = $user->sexe == 0 ? "Men" : "Women" ;?></td>
        <td><?= $user->adresse .", ". $user->ville ." - ". $user->nomPays ;?></td>
        <td>
          <a href="<?= base_url("Admin/Profil/index/".$user->idClient)?>" class="btn btn-info btn-sm">
